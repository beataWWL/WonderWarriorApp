...
export function Nav() {
  ...
  return (
    <nav className="fixed top-0 inset-x-0 z-50 h-[60px] bg-[rgba(0,7,18,0.92)] backdrop-blur border-b border-[rgba(0,212,255,0.15)] px-6 flex items-center gap-0">
      ...
      <Link
        href="/"
        className="font-display uppercase tracking-[0.12em] text-[0.85rem] text-lightning mr-6 whitespace-nowrap flex-shrink-0"
        style={{ textShadow: "0 0 18px rgba(0,212,255,0.5)" }}
      >
        ✦ Wonder
      </Link>

      {/* Class pills — scrollable middle. min-w-0 lets the flex child shrink
          below its intrinsic content width so only THIS section scrolls,
          not the whole nav (which would hide AuthStatus). */}
      ...
      {/* Auth — fixed on the right, never scrolls out of view */}
      <div className="flex-shrink-0 pl-4 ml-2 border-l border-[rgba(0,212,255,0.1)]">
        <AuthStatus />
      </div>
    </nav>
  );
